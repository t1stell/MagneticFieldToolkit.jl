...
md"""
We will load a BMW file with the `read_bmw` command.  Change the path to the BMW file stored locally.
"""
...
md"""
The Poincare plots are produced by seeding the initial positions of the field lines.  The format can either be a `Vector` or `Range` of points specifying in the initial ``(R, Z)`` coordinates of the field lines independently. The `poincare` function will then make a grid of initial points.
"""

# ╔═╡ 6f03c544-822b-42d3-8a77-ea339b44316c
r_range = 7.5:0.01:8.5; z_range = [0.0];

# ╔═╡ 877bc9ca-11fd-47c7-b7af-61dc4d19b492
md"""
The appropriate function to call is the `poincare` function:
```julia
...
```
`ϕ₀` is the intial toroidal angle at which the integration should be started.  The number of toroidal transits is controlled by `trace_ntransits` and the `ϕ_step` keyword controls the maximum timestep of the solver.
"""
...
md"""
The resulting trajectories can be plotted with the `plot_poincare` function:
```julia
